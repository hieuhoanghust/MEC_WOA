...
function [leaderScore, leaderPos, convergenceCurve] = PSO_dl(noSearchAgents, UE_BS, P_SBS_min, P_SBS_max, fobj, X) 
...
%     fobj(P,X);
    ...
    VarMin = P_SBS_min.*UE_BS;         % Lower Bound of Variables
                        % P_SBS_min.*UE_BS
    VarMax = P_SBS_max.*UE_BS;         % Upper Bound of Variables
                        % P_SBS_max.*UE_BS;         % N x M == upper bound
    ...
    for i = 1 : noSearchAgents
        ...
        % Evaluation
        particle(i).Cost    = fobj(particle(i).Position, X);            
