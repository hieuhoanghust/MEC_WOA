...
ALCA.po = zeros(length(M_UL), 1); 
su_ALCA = zeros(length(M_UL), noRealizations); 

ARJOA.po = ones(length(M_UL), 1); 
su_ARJOA = zeros(length(M_UL), noRealizations); 

WOA_BWOA.po = zeros(length(M_UL), 1); 
su_WOA_BWOA = zeros(length(M_UL), noRealizations); 

IOJOA.po = zeros(length(M_UL), 1); 
su_IOJOA = zeros(length(M_UL), noRealizations); 

OFDMA.po = zeros(length(M_UL), 1);
su_OFDMA = zeros(length(M_UL), noRealizations);

PSO_BWOA.po = zeros(length(M_UL), 1);
su_PSO_BWOA = zeros(length(M_UL), noRealizations);

IWOA_BWOA.po = zeros(length(M_UL), 1);
su_IWOA_BWOA = zeros(length(M_UL), noRealizations); 

for iN = 1:length(M_UL)
    M_ul = M_UL(iN);
%     M_dl = M_ul;
    noBSs   = M_ul + M_dl;
    po_WOA_BWOA_iN  = [];
...
save('results\new\Script_M_10_keepDL.mat', 'M_UL', 'M_dl', 'users_no', ...
