...
ALCA.po = zeros(length(F0), 1); 
su_ALCA = zeros(length(F0), noRealizations); 

ARJOA.po = ones(length(F0), 1); 
su_ARJOA = zeros(length(F0), noRealizations); 

WOA_BWOA.po = zeros(length(F0), 1); 
su_WOA_BWOA = zeros(length(F0), noRealizations); 

IOJOA.po = zeros(length(F0), 1); 
su_IOJOA = zeros(length(F0), noRealizations); 

OFDMA.po = zeros(length(F0), 1);
su_OFDMA = zeros(length(F0), noRealizations);

PSO_BWOA.po = zeros(length(F0), 1);
su_PSO_BWOA = zeros(length(F0), noRealizations);

IWOA_BWOA.po = zeros(length(F0), 1);
su_IWOA_BWOA = zeros(length(F0), noRealizations);

setting_ = cell(1, noRealizations);
for iReal = 1:noRealizations
    ...
    setting_{iReal}.UE_BS = UE_BS;
    setting_{iReal}.UEs   = UEs;
    setting_{iReal}.BS    = BS;
     ...
     setting_{iReal}.ChannelGain = ChannelGain;
end
...
for iN = 1:length(F0)
    ...
    for iReal = 1:noRealizations
        fprintf('F0 = %i/%i   iReal = %i/%i\n',F0(iN), F0(length(F0)), iReal, noRealizations)
        UE_BS   = setting_{iReal}.UE_BS;
        UEs     = setting_{iReal}.UEs;
        BS      = setting_{iReal}.BS;
        ChannelGain = setting_{iReal}.ChannelGain;
