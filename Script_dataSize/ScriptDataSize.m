...
ALCA.po = zeros(length(D_n_sample), 1); 
su_ALCA = zeros(length(D_n_sample), noRealizations); 

ARJOA.po = ones(length(D_n_sample), 1); 
su_ARJOA = zeros(length(D_n_sample), noRealizations); 

WOA_BWOA.po = zeros(length(D_n_sample), 1); 
su_WOA_BWOA = zeros(length(D_n_sample), noRealizations); 

IOJOA.po = zeros(length(D_n_sample), 1); 
su_IOJOA = zeros(length(D_n_sample), noRealizations); 

OFDMA.po = zeros(length(D_n_sample), 1);
su_OFDMA = zeros(length(D_n_sample), noRealizations);

PSO_BWOA.po = zeros(length(D_n_sample), 1);
su_PSO_BWOA = zeros(length(D_n_sample), noRealizations);

IWOA_BWOA.po = zeros(length(D_n_sample), 1);
su_IWOA_BWOA = zeros(length(D_n_sample), noRealizations); 

for iN = 1:length(D_n_sample)
	D_n = D_n_sample(iN); 
    D_n = 1e6*D_n; 
    params.D_n = D_n;
    po_WOA_BWOA_iN  = [];
    ...
    for iReal = 1:noRealizations
        ...
        fprintf('iN:%i/%i   iReal:%i/%i\n', iN, length(D_n_sample), iReal, noRealizations);
...
save('results\new\Script_Datasize.mat', 'D_n_sample', 'users_no', ...
